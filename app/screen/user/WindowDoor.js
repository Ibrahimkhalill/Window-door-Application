...
function WindowDoor({ route, navigation }) {
  ...
  const generatePdf = async () => {
    ...
    //   // Create a new file path in the Download directory
    //   const downloadDir = FileSystem.documentDirectory + "Downloads/";
    //   const filePath = downloadDir + `Quotation(${quotationNo}).pdf`;
  ...
  return (
    ...
    >
      {!visibleSave ? (
        <>
          ...
          {visibleContent && (
            <ScrollView style={styles.scrollableContent}>
              <View className="bg-white mt-2 p-2 w-[320] ">
                {windowData.map(
                  (item, index) =>
                    item && (
                      <View
                        key={index}
                        className="bg-background border-[2px] rounded mb-4 w-[305] h-[444] flex items-center"
                      >
                        <Selector
                          label="Design"
                          options={[
                        ...
                        <Selector
                          ...
                          onToggle={(isOpen) =>
                            handleToggle(isOpen ? `mosNetColor${index}` : null)
                          }
                        />
                        <Selector
                          label="Glass Color"
        ...
        <View>
          <View className="bg-white mt-2 h-[500] p-2 w-[330] ">
            <ScrollView style={styles.scrollableContent}>
              {windowData.map((data, index) => (
                <View
                  key={index}
                  className="bg-background flex-col  rounded mb-4 w-[313] h-[210] flex pl-2 pt-2 items-start"
                >
